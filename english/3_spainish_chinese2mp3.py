...
import os
chinese_spainish_xlsx = pandas.read_excel("chinese_spainish.xlsx", header=None)
...
for dataFrameIndex, row in chinese_spainish_xlsx.iterrows():
    ...
    if xlsxIndex>=xlsxIndex_start and xlsxIndex<=xlsxIndex_end:
        ...
        if (dataFrameIndex % 1000) == 0:
            if f is not None:
                f.close()
            mp3Path = "mp3/indexChinese_chinese_spainish_" + str(xlsxIndex) + "_" + str(xlsxIndex + 999) + ".mp3"
